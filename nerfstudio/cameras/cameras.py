...
class Cameras(TensorDataclass):
    ...
    def _generate_rays_from_coords(
        ...
        if CameraType.PANORAMA.value in cam_types:
            ...
            mask = (self.camera_type[true_indices] == CameraType.PANORAMA.value).squeeze(-1)  # (num_rays)
            mask = torch.stack([mask, mask, mask], dim=0)
            ...
            #del xx, yy, zz
            directions_stack[..., 0][mask] = torch.masked_select(torch.sin(theta) * torch.cos(phi), mask).float()
            directions_stack[..., 1][mask] = torch.masked_select(torch.sin(phi), mask).float()
            directions_stack[..., 2][mask] = torch.masked_select(-torch.cos(theta) * torch.cos(phi), mask).float()
